    ...
    if a.status.name!='INFEASIBLE' :
        ...
        for lc in linking_const:    
            ...
            for c in range(len(const_name)):
                linking_result.append((const_name[c][0],lhs[c],const_name[c][1],const_name[c][2], const_stat[c]))


            linking_result_dtype = np.dtype([('c.name', object), ('lhs', object), ('sense', object),('rhs', float) ,('TF', object)])
            linking_result=np.array([(c[0], c[1], c[2], c[3], c[4]) for c in linking_result], dtype = linking_result_dtype)
