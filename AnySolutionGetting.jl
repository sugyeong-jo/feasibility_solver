...
include("AnySolutionGetting_func.jl")
##########################
s = ArgParseSettings()
...
#filename = string("R100701008_2_cplex.mps")
#Sec = 1000
...
# result
#############################################################################
for x in [k for (k,v) in dict_infeasible_index if v==:Bin]
...
#Reupdate(var_lb, dict_xub_k_check)
Set_Type()
#Update(dict_LP_int_check)
#CP(dict_infeasible_index_check)
solution_k, dict_xlb_k,dict_xub_k, dict_LP_int, solution_k_1=LP_solve(m, var_lb, dict_xub_k_check)
termination_status(m)!=MOI.OPTIMAL
##########################################