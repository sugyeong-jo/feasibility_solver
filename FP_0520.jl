...
MathProgBase.loadproblem!(m,"/HDD/Workspace/CLT/mps/processing/CPLEX_file/R100701002_2_cplex.mps")
...
normalized_coefficient(less_than_constraints[1],"NBSBAL_ARBUE0005E_SGSGP0005E_016C1")

constraint_object(less_than_constraints[1]).set

constraint_object(less_than_constraints[1])


dict_var_index_org = Dict()
for i in index_x
    dict_var_index_org[var[i]] = var_org[i]
end


dict_var_org_index = Dict()
for i in index_x
    dict_var_org_index[var_org[i]] = var[i]
end
...
for i in tqdm(keys(dict_IB_type))
    if dict_IB_type[i]==:Bin
        JuMP.unset_binary(dict_var_index_org[i])
    elseif dict_IB_type[i] == :Int
        JuMP.unset_integer(dict_var_index_org[i])
    else
        continue
    end
end
